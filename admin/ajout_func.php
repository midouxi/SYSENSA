...
function login($login,$mdp){
	if($nb=mysql_num_rows(mysql_query("select * from admin where nom_admin='$login'"))>0){
		if($nb=mysql_num_rows($car=mysql_query("select * from admin where nom_admin='$login' and mdp='".mysql_real_escape_string($mdp)."'"))>0){
...
function ajout_professeur($nom,$prenom){
	...
	if(($exists=mysql_num_rows($prof_exist))>0){
		...
		$p=preg_split("/[a-z]/",$prenom);$pass=strtolower($p[0].$nom);
...
function ajout_eleve($nom,$prenom,$classe){
	...
	if(($exists=mysql_num_rows($eleve_exist))>0){
		...
		return $nom." ".$prenom." existe deja !! Il a le num&eacute;ro ".$ligne[0]."<br>";
	}else{
		if($eleve=mysql_query("INSERT into eleve(nom_eleve,prenom_eleve,id_classe) values('$nom','$prenom','$classe')")) 
		return "<b>Insertion de l'&eacute;l&egrave;ve ".$nom." ".$prenom." dans la classe ".$idclasse[1]." r&eacute;ussie</b><br>"; else return mysql_error();
	}
}
...
function ajout_enseigne($nom,$prenom,$matiere,$coef,$classe){
	...
	$enseigne_exist=mysql_query("select * from ENSEIGNE where id_classe='$idclasse[0]' and id_professeur='$idprofesseur' and id_matiere='$id_matiere[0]'");
        if(($exists=mysql_num_rows($enseigne_exist))>0){
        	...
        	return $nom." ".$prenom." enseigne deja la matiere ".$idmatiere[1]." a la classe ".$classe[1];
        }else{
